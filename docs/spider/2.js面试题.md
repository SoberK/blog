...
### [#](3-JS模块.html#_2-1-typeof类型判断) 2.1 typeof类型判断
...
![](https://www.123fe.net/fe-api/images/20210309102100.png)
...
### [#](3-JS模块.html#_6-3-方式3-将前两种组合) 6.3 方式3：将前两种组合
...
![](https://www.123fe.net/fe-api/uploads/2022/08/7256bc5207e91265.png)
...
### [#](3-JS模块.html#_12-4-settimeout、promise、async-await-的区别) 12.4
...
### [#](3-JS模块.html#_12-5-settimeout-fn-0-多久才执行-event-loop) 12.5
setTimeout(fn, 0)多久才执行，Event Loop

> setTimeout 按照顺序放到队列里面，然后等待函数调用栈清空之后才开始执行，而这些操作进入队列的顺序，则由设定的延迟时间来决定

### [#](3-JS模块.html#_12-6-async原理) 12.6 async原理
...
## [#](3-JS模块.html#_13-内存泄露) 13 内存泄露

### [#](3-JS模块.html#_13-1-chrome-devtools查看内存情况) 13.1 Chrome devTools查看内存情况
...
![](https://www.123fe.net/fe-api/uploads/2022/08/d1451162abd9747e.png)

简单录制一下百度页面，看看我们能获得什么，如下动图所示：

![](https://www.123fe.net/fe-api/uploads/2022/08/e683158f36977b6b.gif)
...
![](https://www.123fe.net/fe-api/uploads/2022/08/ef508139131f6abc.png)

堆快照就像照相机一样，能记录你当前页面的堆内存情况，每快照一次就会产生一条快照记录

![](https://www.123fe.net/fe-api/uploads/2022/08/e2a1fe0d0aa4d4fd.png)
...
![](https://www.123fe.net/fe-api/uploads/2022/08/ee6a489b7e87ef0f.png)
...
![](https://www.123fe.net/fe-api/uploads/2022/08/bcb184db6f3912da.png)

### [#](3-JS模块.html#_13-2-内存泄漏的场景) 13.2 内存泄漏的场景
...
![](https://www.123fe.net/fe-api/uploads/2022/08/70ebeb00bb65e063.gif)
...
![](https://www.123fe.net/fe-api/uploads/2022/08/3c5e8a655b107d85.gif)
...
![](https://www.123fe.net/fe-api/uploads/2022/08/89b88d985b2b101e.gif)
...
![](https://www.123fe.net/fe-api/uploads/2022/08/3ab5821936052270.gif)
...
![](https://www.123fe.net/fe-api/uploads/2022/08/1fa90928bddb4112.gif)
...
![](https://www.123fe.net/fe-api/uploads/2022/08/7295a733de9b6e6d.gif)
...
![](https://www.123fe.net/fe-api/uploads/2022/08/b875ced23a8c96d7.gif)
...
![](https://www.123fe.net/fe-api/uploads/2022/08/da2f80dd5bad24b2.gif)

注释掉了`console.log`

![](https://www.123fe.net/fe-api/uploads/2022/08/a69e5d3e85c60164.gif)
...
![](https://www.123fe.net/fe-api/uploads/2022/08/babc2efb74c7ed87.gif)

按道理来说点击按钮执行`fn1`函数后会退出该函数的执行上下文，紧跟着函数体内的局部变量应该被清除，但图中`performance`的录制结果显示似乎是存在内存泄漏问题的，即最终曲线高度比基准线高度要高，那么再用`Memory`来确认一次：

![](https://www.123fe.net/fe-api/uploads/2022/08/2391b90e30911564.gif)
...
![](https://www.123fe.net/fe-api/uploads/2022/08/a55f5e42812ae5a2.gif)

> 这次的录制结果就能看出，最后的曲线高度和初始基准线的高度一样，说明并没有内存泄漏的情况

  * `memory`

![](https://www.123fe.net/fe-api/uploads/2022/08/3c69cab609414411.gif)
...
## [#](3-JS模块.html#_14-垃圾回收机制) 14 垃圾回收机制
...
**标记清除的缺点：**
  ...
  * **内存碎片化** ，空闲内存块是不连续的，容易出现很多空闲内存块，还可能会出现        分配所需内存过大的对象时找不到合适的块。
  * **分配速度慢** ，因为即便是使用 First-fit 策略，其操作仍是一个 `O(n)` 的操作，最坏情况是每次都要遍历到最后，同时因为碎片化，大对象的分配效率会更慢。

> 解决以上的缺点可以使用 **标记整理（Mark-Compact）算法**
> 标记结束后，标记整理算法会将活着的对象（即不需要清理的对象）向内存的一端移动，最后清理掉边界的内存（如下图）

![](https://www.123fe.net/fe-api/uploads/2022/08/9ab816979f615b6e.png)
...
## [#](3-JS模块.html#_15-深浅拷贝) 15 深浅拷贝

![](https://www.123fe.net/fe-api/images/20210414142630.png)
...
![](https://www.123fe.net/fe-api/images/20210414134752.png)
...
![](https://www.123fe.net/fe-api/images/20210414141731.png)
...
![](https://www.123fe.net/fe-api/uploads/2022/08/f636f4c6e3cfbd36.png)

## [#](3-JS模块.html#_16-对象的几种创建方式) 16 对象的几种创建方式

### [#](3-JS模块.html#_16-1-工厂模式-创建方式) 16.1 工厂模式,创建方式
...
### [#](3-JS模块.html#_16-2-构造函数模式) 16.2 构造函数模式
...
### [#](3-JS模块.html#_16-3-使用原型模式) 16.3 使用原型模式
...
### [#](3-JS模块.html#_16-4-组合使用构造函数模式和原型模式) 16.4 组合使用构造函数模式和原型模式
...
### [#](3-JS模块.html#_16-5-动态原型模式) 16.5 动态原型模式
...
## [#](3-JS模块.html#_17-数组相关) 17 数组相关

### [#](3-JS模块.html#_17-1-数组常用方法) 17.1 数组常用方法
...
### [#](3-JS模块.html#_17-2-array-3-和array-3-4-的区别) 17.2 Array(3)和Array(3,
4)的区别？
...
### [#](3-JS模块.html#_17-3-请创建一个长度为100-值都为1的数组) 17.3 请创建一个长度为100，值都为1的数组
...
### [#](3-JS模块.html#_17-4-请创建一个长度为100-值为对应下标的数组) 17.4 请创建一个长度为100，值为对应下标的数组
...
### [#](3-JS模块.html#_17-5-如何转化类数组成数组) 17.5 如何转化类数组成数组
...
### [#](3-JS模块.html#_17-6-foreach中return有效果吗-如何中断foreach循环) 17.6
...
### [#](3-JS模块.html#_17-7-js判断数组中是否包含某个值) 17.7 JS判断数组中是否包含某个值
...
### [#](3-JS模块.html#_17-8-js中flat-数组扁平化) 17.8 JS中flat---数组扁平化
...
## [#](3-JS模块.html#_18-操作dom) 18 操作DOM

### [#](3-JS模块.html#_18-1-说说有几种类型的dom节点) 18.1 说说有几种类型的DOM节点
...
### [#](3-JS模块.html#_18-2-操作dom节点方法) 18.2 操作DOM节点方法
...
## [#](3-JS模块.html#_19-ajax总结) 19 Ajax总结
...
### [#](3-JS模块.html#_19-1-ajax-有那些优缺点) 19.1 Ajax 有那些优缺点
...
### [#](3-JS模块.html#_19-2-关于http-xmlhttprequest-ajax的关系) 19.2
...
### [#](3-JS模块.html#_19-3-xmlhttprequest的发展历程是怎样的) 19.3
...
### [#](3-JS模块.html#_19-4-使用xmlhttprequest封装一个get和post请求) 19.4
...
## [#](3-JS模块.html#_20-定时器) 20 定时器

### [#](3-JS模块.html#_20-1-setinterval存在哪些问题) 20.1 setInterval存在哪些问题？
...
### [#](3-JS模块.html#_20-2-链式调用settimeout对比setinterval) 20.2
...
### [#](3-JS模块.html#_20-3-实现比-settimeout-快-80-倍的定时器) 20.3 实现比 setTimeout 快 80
倍的定时器
...
### [#](3-JS模块.html#_22-4-说一下requestanimationframe) 22.4
...
### [#](3-JS模块.html#_22-5-requestanimationframe对比settimeout) 22.5
...
## [#](3-JS模块.html#_21-谈谈你对for-in-for-of的理解) 21 谈谈你对for in/for of的理解
...
![](https://www.123fe.net/fe-api/images/20210320102041.png)
![](https://www.123fe.net/fe-api/images/20210320103701.png)
...
## [#](3-JS模块.html#_22-javascript-实现对上传图片的压缩) 22 JavaScript 实现对上传图片的压缩？
